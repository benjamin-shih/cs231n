...
def compute_saliency_maps(X, y, model):
    ...
    grad = tf.abs(tape.gradient(loss, X))
    saliency = tf.reduce_max(grads, axis = 3)
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        ...
        X_fooling += learning_rate * grad /tf.norm(grad, ord = 'euclidean')
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        tape.watch(X)
        scores = model(X)
        correct_scores = scores[0, target_y]
        loss = correct_scores - l2_reg * tf.norm(X)**2
