...
def conv_forward_naive(x, w, b, conv_param):
    ...
    assert (H - F + 2 * pad)%S = 0
    assert (W - F + 2 * pad)%S = 0
    #output width and height
    H_out = 1 + (H + 2 * pad - HH)//stride
    W_out = 1 + (W + 2 * pad - WW)//stride
    #populate output matrix size with zeros
    ...
    #im2col
    x_col = np.zeros((HH * WW * C, H_out * W_out))
    for index in range(N):
        ...
        for i in range(0, H_pad - HH + 1, stride):
            for j in range(0, W_pad - WW + 1, stride):
                ...
                neuron = neuron + 1
        out[index] = ((W_row.dot(x_col)) + b.reshape((F, 1))).reshape(F, HH, WW)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    dx = np.zeros_like((N, C, H_pad - 2*pad, W_pad - 2*pad)) #[N, C, H, W]
    db = np.zeros_like(b) #[F, ]

    #implement im2col for backwards convolutional layer
    w_row = w.reshape((F, C * HH * WW))
    x_col = np.zeros((C*HH*WW, outH* outW)) #[C*HH*WW x H'*W']
    for index in range(N):
        out_col = dout[index].reshape(F, outH * outW)
        w_out = w_row.T.ot(out_col)
        ...
        dw = dw + out_col.dot(x.col.T).reshape(F, C, HH, WW)
        db = db + out_col.sum(axis = 1)
    ...
    for index in range(N):
        ...
        for i in range(0, H - ph + 1, stride):
            for j in range(0, W - pw + 1, stride):
                pool_reg = x[index, :, i:i + h_out, j:j + w_out].reshape(C, ph, pw)
                out_col[:, n] = pool_reg.max(axis = 1)
                n = n + 1
